...
import style from './BaiTapRenderDanhSachPhim.css'
const mangPhim =
    [
        {
            maPhim: 1321,
    ...
    renderPhimWithMap = () => {
        const arrJSX = mangPhim.map((phim, index) => {
            return <div className="col-4" key={index}>
                <div className="card">
                    <img style={this.objectStyle} className="card-img-top" src={phim.hinhAnh} alt="123" />
                    <div className="card-body">
                        <h4 className="card-title" style={{ height: "60px" }}>{phim.tenPhim > 50 ? <span>{phim.tenPhim.substr(0, 50)}...</span> : <span>{phim.tenPhim}</span>}</h4>
